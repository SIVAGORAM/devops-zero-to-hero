...
## 🧑‍💻 2. The Complete End-to-End User Creation Lab
...
# 5. The Cluster Admin approves the request
kubectl certificate approve Siva-csr

# 6. Extract the newly signed Certificate
...
We can use `auth can-i` to impersonate Siva and test his exact permissions!

```bash
# Test 1: Can Siva read pods in Dev? (Should return 'yes')
kubectl --kubeconfig=Siva-kubeconfig.yaml auth can-i get pods -n dev

# Test 2: Can Siva DELETE pods in Dev? (Should return 'no')
kubectl --kubeconfig=Siva-kubeconfig.yaml auth can-i delete pods -n dev

# Test 3: Can Siva read pods in the Default namespace? (Should return 'no')
...
| Concept | Simple Meaning | Real-World Use Case |
| :--- | :--- | :--- |
| **Role** | Permissions in ONE namespace | "Can read Pods in the Dev namespace." |
| **ClusterRole** | Permissions across the CLUSTER | "Can read Pods in Dev, QA, and Prod namespaces." |
| **RoleBinding** | Assigns a Role to a User | "Give Siva the Dev-Reader Role." |
| **Kubeconfig** | The file used to connect to K8s | "Here is your login file, Siva. Place it in `~/.kube/config`." |
...
> [!IMPORTANT]
> **Gotcha 3: The God-Mode File (dmin.conf)**
