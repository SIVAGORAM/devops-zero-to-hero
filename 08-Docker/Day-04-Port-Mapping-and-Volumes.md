...
Imagine you install an NGINX web server directly on your Ubuntu EC2 instance on Port 80 using these commands:
```bash
sudo su
apt install nginx -y
cd /var/www/html # Add some custom HTML here
```
If you open port 80 in your AWS Security Group, anyone on the internet can type your IP address and see the website. 
...
*2. **Map to a different Host Port (The Docker Way):** I can simply map an empty host port (like 9090) to the container's 8080 port by running `docker run -p 9090:8080 jenkins`. The users will just access the app via port 9090! (Remember: One host port can only be used once at a time)."*

### Bonus: Hosting Node-RED
Just to prove how easy it is to host any application once you know port mapping, you can host Node-RED instantly:
```bash
docker run -d -p 1880:1880 nodered/node-red
```
Just open port 1880 in your AWS Security Group, and you have a live Node-RED server!

---

## 📝 A Quick Note on Logs
Before we jump into Volumes, remember that containers are ephemeral. If a container dies, you lose its logs. 
There are generally 2 types of logs you need to care about:
1. **Application Logs:** Generated by the code running inside (e.g., Python errors, Java stack traces).
2. **Server Logs:** Generated by the infrastructure (e.g., NGINX access logs).
Because containers wipe out data when they die, we use **Volumes** to save these logs permanently!
